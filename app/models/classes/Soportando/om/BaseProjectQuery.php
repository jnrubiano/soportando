 ...
 *
 * @method ProjectQuery leftJoinPackage($relationAlias = null) Adds a LEFT JOIN clause to the query using the Package relation
 * @method ProjectQuery rightJoinPackage($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Package relation
 * @method ProjectQuery innerJoinPackage($relationAlias = null) Adds a INNER JOIN clause to the query using the Package relation
 *
...
abstract class BaseProjectQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'Soportando', $modelName = 'Project', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `project_id`, `name` FROM `project` WHERE `project_id` = :p0';
        try {
     ...
     *
     * @param   Package|PropelObjectCollection $package  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 ProjectQuery The current query, for fluid interface
    ...
    public function filterByPackage($package, $comparison = null)
    {
        if ($package instanceof Package) {
            return $this
                ->addUsingAlias(ProjectPeer::PROJECT_ID, $package->getProjectId(), $comparison);
